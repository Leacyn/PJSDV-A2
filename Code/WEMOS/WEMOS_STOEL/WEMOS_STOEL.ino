...
void wijzigTril()
{
  ...
  // Begin transmissie vibratie motor
  Wire.beginTransmission(0x38);
  Wire.write(byte(0x03));
  Wire.write(byte(0x0F));
  Wire.endTransmission();

  // Zet vibratie motor op basis van ontvangen state van de PI
  Wire.beginTransmission(0x38);
  Wire.write(byte(0x01));
  Wire.write(byte((Tril.state + Led.state) << 4));  // Omdat beide actuatoren op hetzelfde adres zitten, moeten ook beide states bij elkaar opgeteld worden
  Wire.endTransmission();
}
...
void wijzigLed()
{
  ...
  Wire.beginTransmission(0x38);
  Wire.write(byte(0x01));
  Wire.write(byte((Led.state + Tril.state) << 4)); // Omdat beide actuatoren op hetzelfde adres zitten, moeten ook beide states bij elkaar opgeteld worden
  Wire.endTransmission();
}
...
void leesDruksensor()
{
  // Begin trnasmissie met druksensor
  ...
  drukSensor.state = (anin0 / 100); // uitgelzen waarde 0 t/m 10
  ...
  if (Ontvangst.cmd == "r") {
    ...
    switch (Ontvangst.id) {
      ...
      case 4:
        leesDruksensor();
        Serial.print("sending state:");
        sending = SendJson(drukSensor);   // Stuur data van drukSensor naar PI
        Serial.print(sending);
        break;
      case 5:
        Serial.print("sending state:");
